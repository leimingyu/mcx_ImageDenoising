...
##
##
##    with tf.variable_scope('mid1'):
##		mid1 = input - output
##
##    with tf.variable_scope('block18'):
##        output = tf.layers.conv2d(mid1, 64, 3, padding='same', activation=tf.nn.relu)
##
##    for layers in xrange(19, 22 + 1):
##        with tf.variable_scope('block%d' % layers):
##            output = tf.layers.conv2d(output, 64, 3, padding='same', name='conv%d' % layers, use_bias=False)
##            output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
##    with tf.variable_scope('block23'):
##        output = tf.layers.conv2d(output, output_channels, 3, padding='same')
##
##    return input - output
...
##    # 18 to 19 
##    with tf.variable_scope('block18'):
##        output = tf.layers.conv2d(mid1, 64, 3, padding='same', name='conv18', use_bias=False)
##        output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
##
##    with tf.variable_scope('block19'):
##        output = tf.layers.conv2d(output, 64, 3, padding='same', name='conv19', use_bias=False)
##        output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
##
##    with tf.variable_scope('mid2'):
##        mid2 = mid1 - output 
##
##    # 20 to 21 
##    with tf.variable_scope('block20'):
##        output = tf.layers.conv2d(mid2, 64, 3, padding='same', name='conv20', use_bias=False)
##        output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
##
##    with tf.variable_scope('block21'):
##        output = tf.layers.conv2d(output, 64, 3, padding='same', name='conv21', use_bias=False)
##        output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
##
##    with tf.variable_scope('mid3'):
##        mid3 = mid2 - output 
##
##    # 22 to 23 
##    with tf.variable_scope('block22'):
##        output = tf.layers.conv2d(mid3, 64, 3, padding='same', name='conv22', use_bias=False)
##        output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
##
##    with tf.variable_scope('block23'):
##        output = tf.layers.conv2d(output, 64, 3, padding='same', name='conv23', use_bias=False)
##        output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
##
##    return input - output
...
class denoiser(object):
    ...
    def test(self, noisydata_norm, ckpt_dir):
        """Test DnCNN"""
        # init variables
