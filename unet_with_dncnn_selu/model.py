...
def unet(input, is_training=True, output_channels=1):
    ...
    with tf.variable_scope('block1'):
        ...
        output = tf.layers.conv2d(input, 64, 3, padding='same', activation=selu)
    for layers in xrange(2, 5):
        with tf.variable_scope('block%d' % layers):
            output = tf.layers.conv2d(output, 64, 3, padding='same', name='conv%d' % layers, use_bias=False)
            #output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
            output = selu(tf.layers.batch_normalization(output, training=is_training))
