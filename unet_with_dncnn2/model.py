...
def unet(input, is_training=True, output_channels=1):
    # cnn0
    with tf.variable_scope('cnn0-block1'):
        output = tf.layers.conv2d(input, 64, 3, padding='same', activation=tf.nn.relu)
    for layers in xrange(2, 4):
        with tf.variable_scope('cnn0-block%d' % layers):
            output = tf.layers.conv2d(output, 64, 3, padding='same', name='conv%d' % layers, use_bias=False)
            output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
    with tf.variable_scope('cnn0-block4'):
    ...
    with tf.variable_scope('cnn1'):
        input_cnn1 = input - output

    with tf.variable_scope('cnn1-block1'):
        output = tf.layers.conv2d(input_cnn1, 64, 3, padding='same', activation=tf.nn.relu)
    for layers in xrange(2, 4):
        with tf.variable_scope('cnn1-block%d' % layers):
            output = tf.layers.conv2d(output, 64, 3, padding='same', name='conv%d' % layers, use_bias=False)
            output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
    with tf.variable_scope('cnn1-block4'):
        output = tf.layers.conv2d(output, output_channels, 3, padding='same')
    ...
    with tf.variable_scope('cnn2'):
        input_cnn2 = input_cnn1 - output

    with tf.variable_scope('cnn2-block1'):
        output = tf.layers.conv2d(input_cnn2, 64, 3, padding='same', activation=tf.nn.relu)
    for layers in xrange(2, 4):
        with tf.variable_scope('cnn2-block%d' % layers):
            output = tf.layers.conv2d(output, 64, 3, padding='same', name='conv%d' % layers, use_bias=False)
            output = tf.nn.relu(tf.layers.batch_normalization(output, training=is_training))
    with tf.variable_scope('cnn2-block4'):
        output = tf.layers.conv2d(output, output_channels, 3, padding='same')
